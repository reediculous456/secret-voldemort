...
module.exports.setVerify = ({ username, email, res, isResetPassword }) => {
	...
	verify.save(() => {
		...
		nmMailgun.sendMail({
			from: 'SV.io accounts <donotreply@secretvoldemort.io>',
			html: isResetPassword ? resetTemplate({ username, token }) : verifyTemplate({ username, token }),
			text: isResetPassword
				? `Hello ${username}, a request has been made to change your password - go to the address below to change your password. https://secretvoldemort.io/reset-password/${username}/${token}.`
				: `Hello ${username}, a request has been made to verify your account - go to the address below to verify it. https://secretvoldemort.io/verify-account/${username}/${token}`,
			to: email,
			subject: isResetPassword ? 'SH.io - reset your password' : 'SH.io - verify your account'
		});

		// nmMailgun.sendMail({
		// 	from: 'Secret Voldemort.io <donotreply@secretvoldemort.io>',
		// 	html: isResetPassword ? resetTemplate({ username, token }) : verifyTemplate({ username, token }),
		// 	to: email,
		// 	subject: 'Secret Voldemort IO - verify your account',

			...
			return gameList
				.filter(game => {
					...
					return !(
						...
						((game.gameStatus === 'death-eater' || game.gameStatus === 'order') && completed) ||
						(game.isCustomGame && customgame) ||
						(game.casualGame && casualgame)
					);
				})
				.sort((a, b) => {
					const statusSortOrder = ['notStarted', 'isStarted', 'death-eater', 'order'];
			...
			<section className={this.state.filtersVisible ? 'browser-container' : 'browser-container filters-hidden'}>
				<a href="#/changelog">
					<h5 title="A season is an optional new tier of elo that is reset every 3 months.">
						{new Date() > new Date('2019-12-31') ? `Season ends ${moment(new Date('2021-12-31')).fromNow()}` : `Welcome to season ${CURRENTSEASONNUMBER}`}.
					</h5>
				</a>
				<h3>Game filters</h3>
				{this.renderFilters()}

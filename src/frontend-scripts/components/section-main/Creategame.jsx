...
export default class Creategame extends React.Component {
	...
	renderDeck() {
		...
		const data = _.range(0, numLib)
			.map((val, i) => {
				return <div className="deckcard" key={i} style={{ backgroundImage: "url('../images/cards/liberalp-l.png')" }} />;
			})
			.concat(
				_.range(0, numFas).map((val, i) => {
					return <div className="deckcard" key={i} style={{ backgroundImage: "url('../images/cards/fascistp-l.png')" }} />;
				})
			);
		const thirds = [];
		...
		return thirds.map((val, i) => {
			return (
				<div className="column" key={i} style={{ width: '4em', marginRight: '-1em' }}>
					{val}
				</div>
			);
		});
